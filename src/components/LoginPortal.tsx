...
export type UserRole = 'guard' | 'operations' | 'incident' | 'executive' | 'admin'
    ...
    <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
          <label style={{ fontFamily: 'Inter', fontSize: 12, fontWeight: 500, color: '#dfe7ff' }}>Password</label>
          <button type="button" onClick={() => setHelpMessage('Password recovery instructions will be sent to your registered account.')} style={{ fontFamily: 'Inter', fontSize: 11, color: accent, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>Forgot?</button>
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '4px 0' }}>
        ...
        <span style={{ fontFamily: 'Inter', fontSize: 11, color: '#dfe7ff' }}>or</span>
        <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.07)' }} />
      </div>
      <div style={{ textAlign: 'center' }}>
        <span style={{ fontFamily: 'Inter', fontSize: 13, color: '#dfe7ff' }}>No account? </span>
        <button type="button" onClick={() => setHelpMessage('Access request form is ready. Contact your system administrator to complete approval.')} style={{ fontFamily: 'Inter', fontSize: 13, fontWeight: 700, color: accent, background: 'none', border: 'none', cursor: 'pointer' }}>Request Access</button>
      </div>
    </form>
  )
}
...
function GuardLogin({ onBack, onLogin }: { onBack: () => void; onLogin: () => void }) {
  return (
    <div style={{ minHeight: '100vh', background: 'rgba(6,14,9,0.88)', display: 'flex', position: 'relative', overflow: 'hidden' }}>
      {/* Hex-grid watermark */}
      <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', opacity: 0.04, pointerEvents: 'none' }} viewBox="0 0 400 400">
        {Array.from({ length: 8 }).map((_, row) =>
          Array.from({ length: 8 }).map((_, col) => {
            ...
            return <polygon key={`${row}-${col}`} points={pts} fill="none" stroke="#22c55e" strokeWidth="1" />
          })
        )}
      </svg>

      {/* Left panel – branding */}
      <div style={{ width: '42%', background: 'rgba(34,197,94,0.06)', borderRight: '1px solid rgba(34,197,94,0.15)', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '60px 48px', position: 'relative' }}>
        <Logos size={44} />
        <div style={{ marginTop: 32 }}>
          <div style={{ fontFamily: 'JetBrains Mono', fontSize: 10, color: '#22c55e', letterSpacing: '0.2em', marginBottom: 12 }}>SECURITY PERSONNEL</div>
          <h1 style={{ fontFamily: 'Inter', fontWeight: 800, fontSize: 32, color: '#e8eaf0', margin: '0 0 14px', lineHeight: 1.1 }}>Guard<br />Portal</h1>
        ...
        <div style={{ position: 'absolute', bottom: 28, left: 48, fontFamily: 'JetBrains Mono', fontSize: 9, color: 'rgba(34,197,94,0.3)', letterSpacing: '0.1em' }}>
          BCC/CAT SECURITY · GUARD ACCESS
        </div>
      </div>

      {/* Right panel – form */}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '60px 56px' }}>
        <button onClick={onBack} style={{ position: 'absolute', top: 28, right: 28, display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'Inter', fontSize: 12, color: 'rgba(34,197,94,0.5)', background: 'none', border: 'none', cursor: 'pointer' }}>
          ← All Portals
        </button>
        <div style={{ maxWidth: 360, width: '100%' }}>
          <div style={{ display: 'inline-block', fontFamily: 'JetBrains Mono', fontSize: 10, color: '#22c55e', letterSpacing: '0.15em', background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.25)', borderRadius: 4, padding: '4px 10px', marginBottom: 20 }}>GUARD LOGIN</div>
          ...
          <FormFields accent="#22c55e" onLogin={onLogin} submitLabel="Start My Duty Shift" inputBg="rgba(34,197,94,0.05)" inputBorder="rgba(34,197,94,0.2)" />
        </div>
      </div>
    </div>
  )
    ...
    <div style={{ minHeight: '100vh', background: darkMode ? "linear-gradient(160deg, rgba(8, 10, 24, 0.78), rgba(20, 13, 48, 0.9)), url('/assets/bg.jpg') center / cover fixed" : 'linear-gradient(180deg, #f4fbff 0%, #d9effc 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden', padding: 24 }}>
      ...
      <div style={{ width: '100%', maxWidth: 480, background: 'rgba(13,10,30,0.75)', border: '1px solid rgba(240,101,34,0.2)', borderRadius: 16, overflow: 'hidden', boxShadow: '0 0 60px rgba(240,101,34,0.12)' }}>
        ...
        <div style={{ padding: '32px' }}>
          <FormFields accent="#F06522" onLogin={onLogin} submitLabel="Access Operations Portal" inputBg="#0D0B1E" inputBorder="rgba(255,255,255,0.1)" />
        </div>
      </div>

      <div style={{ display: 'flex', gap: 20, marginTop: 28 }}>
        {['Guard Roster', 'Deployment Plans', 'Attendance Logs', 'Tactical Room'].map(a => (
...
// ══════════════════════════════════════════════════════════════════════════════
// PORTAL 3 — Incident Management (amber alert)
// ══════════════════════════════════════════════════════════════════════════════
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 24px' }}>
        ...
        <div style={{ width: '100%', maxWidth: 400 }}>
          ...
          <div style={{ background: 'rgba(26,8,8,0.92)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 12, padding: '28px', boxShadow: '0 0 40px rgba(239,68,68,0.1)' }}>
            ...
            <FormFields accent="#ef4444" onLogin={onLogin} submitLabel="Authenticate & Enter" inputBg="rgba(239,68,68,0.04)" inputBorder="rgba(239,68,68,0.2)" />
          </div>
...
const cards: PortalCard[] = [
  {
    id: 'guard', title: 'Security Personnel', subtitle: 'Guard Portal',
  ...
  },
  {
    ...
    access: ['Guard Roster', 'Deployment Plans', 'Attendance Logs', 'Tactical Room'],
  },
  {
    id: 'incident', title: 'Incident Management', subtitle: 'Case Portal',
    color: '#f59e0b', bg: 'rgba(245,158,11,0.08)',
    icon: <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>,
    access: ['Incident Register', 'Case Investigation', 'Approvals', 'Evidence Logs'],
  },
  {
    id: 'executive', title: 'Management / Executives', subtitle: 'Command Portal',
    color: '#60a5fa', bg: 'rgba(96,165,250,0.08)',
    icon: <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg>,
    access: ['KPI Dashboard', 'SLA Reports', 'Analytics', 'Executive Reports'],
  },
  {
...
export default function LoginPortal({ onLogin, darkMode, onToggleTheme }: Props) {
  const [selected, setSelected] = useState<UserRole | null>(null)

  const goBack = () => setSelected(null)

  if (selected === 'guard')     return <GuardLogin     onBack={goBack} onLogin={() => onLogin('guard')} />
  if (selected === 'operations') return <OpsLogin      darkMode={darkMode} onToggleTheme={onToggleTheme} onBack={goBack} onLogin={() => onLogin('operations')} />
  if (selected === 'incident')  return <IncidentLogin  onBack={goBack} onLogin={() => onLogin('incident')} />
  if (selected === 'executive') return <ExecutiveLogin onBack={goBack} onLogin={() => onLogin('executive')} />
  if (selected === 'admin')     return <AdminLogin     onBack={goBack} onLogin={() => onLogin('admin')} />
  ...
  return (
    <div style={{ minHeight: '100vh', background: darkMode ? "linear-gradient(160deg, rgba(8, 10, 24, 0.78), rgba(20, 13, 48, 0.9)), url('/assets/bg.jpg') center / cover fixed" : 'linear-gradient(180deg, #f4fbff 0%, #d9effc 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '52px 24px 40px', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 14, width: '100%', maxWidth: 1120, position: 'relative', zIndex: 1 }}>
        {cards.map(c => (
          <button key={c.id} onClick={() => setSelected(c.id)}
            ...
            onMouseEnter={e => { e.currentTarget.style.borderColor = `${c.color}66`; e.currentTarget.style.background = `linear-gradient(180deg, ${c.bg} 0%, rgba(12,16,26,0.96) 100%)`; e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.boxShadow = `0 24px 48px ${c.color}22` }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = darkMode ? 'rgba(148,163,184,0.16)' : 'rgba(54,126,171,0.18)'; e.currentTarget.style.background = darkMode ? 'rgba(15,23,42,0.82)' : 'rgba(255,255,255,0.82)'; e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = darkMode ? '0 18px 40px rgba(2,6,23,0.36)' : '0 14px 30px rgba(54,126,171,0.16)' }}
          >
            <div style={{ width: 50, height: 50, borderRadius: 13, background: `${c.color}18`, border: `1px solid ${c.color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: c.color, marginBottom: 18, boxShadow: `inset 0 0 0 1px ${c.color}22` }}>
              {c.icon}
            </div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: 18 }}>
              {c.access.map(a => (
                <span key={a} style={{ fontFamily: 'Inter', fontSize: 10, color: darkMode ? '#cfe7ff' : '#52718b', background: darkMode ? 'rgba(255,255,255,0.06)' : '#e7f5fd', border: darkMode ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(54,126,171,0.12)', borderRadius: 20, padding: '2px 9px' }}>{a}</span>
              ))}
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontFamily: 'Inter', fontSize: 12, fontWeight: 600, color: c.color }}>
              Sign in
