...
export default function EquipmentModule({ canEdit = false, canReport = false }: { canEdit?: boolean; canReport?: boolean }) {
  ...
  const downloadEquipment = () => downloadPdf('bcc-cat-equipment-inventory.pdf', 'Equipment Inventory', ['Asset ID', 'Model', 'Serial', 'Assigned To', 'Site', 'Status'], [...firearms.map(item => [item.id, item.model, item.serial, item.assignee, item.site, item.status]), ...radios.map(item => [item.id, item.model, item.serial, item.assignee, item.site, item.status])])
